...
class ProfileController {
  constructor($scope, $state, $stateParams, $ngRedux, allergiesActions, serviceRequests, usSpinnerService, $timeout) {
      ...
      if ($scope.logoFileParams.name) {
        this.getBase64Image($scope.logoFileParams.path, $scope.logoFileParams.file, function (dataURL) {
          ...
          if (appSettingsForm.$valid) {
            $scope.isAppSettingsEdit = false;
            this.appSettings = Object.assign({}, $scope.appSettingsEdit);
...
ProfileController.$inject = ['$scope', '$state', '$stateParams', '$ngRedux', 'allergiesActions', 'serviceRequests', 'usSpinnerService', '$timeout'];
